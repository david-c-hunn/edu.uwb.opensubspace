...
do
	...
	do
		...
		do
			...
			for j in {1..6}
			do
				echo "Running CLIQUE with XI=${XI} and TAU=${TAU}..."
				#echo "java -Xmx1024m -cp $class_path -sc $clusterer -t $in_file -T $true_file -c last -M $metrics -XI $XI -TAU $TAU -timelimit 1 >> $outfile"
