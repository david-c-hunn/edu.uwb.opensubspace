			...
			for j in {1..4}; do
				...
				for kk in {1..6}; do
					...
					for l in {1..3}; do
						...
						java -Xmx1024m -cp $class_path -sc $clusterer -t $in_file -T $true_file -c last -M $metrics -timelimit 30 -a $ALPHA -b $BETA -m $MAXITER -k $K -w $W -outfile $outfile
