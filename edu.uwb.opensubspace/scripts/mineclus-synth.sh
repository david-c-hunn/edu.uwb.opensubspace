...
class_path="i9-weka.jar:weka.jar:i9-subspace.jar:Jama.jar:jsc.jar:commons-math-1.1.jar:vecmath.jar:j3dcore.jar:j3dutils.jar weka.subspaceClusterer.Evaluator"
...
for db in $dbs; do
	for in_file in $db; do
		...
		outfile="output/${clusterer}-synth-2"
			...
			for j in {1..4}; do
				...
				for kk in {1..6}; do
					...
					for l in {1..3}; do
						...
						java -Xmx1024m -cp $class_path -sc $clusterer -t $in_file -T $true_file -c last -M $metrics -timelimit 30 -a $ALPHA -b $BETA -m $MAXOUT -k $K -n $NUM_BINS -w $W  -outfile $outfile
