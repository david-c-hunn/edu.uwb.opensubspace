...
for db in $dbs; do
	for in_file in $db; do
		...
		outfile="output/${clusterer}-real-disjoint-test"
		...
		for i in {1..3}; do
			...
			for j in {1..4}; do
				...
				for l in {1..3}; do
					...
					java -Xmx1024m -cp $class_path -sc $clusterer -t $in_file -T $true_file -c last -M $metrics -timelimit 30 -a $ALPHA -b $BETA -w $W -e $EPSILON -m $MU_0 -n 0 -s $MIN_SUBSPACE -x true >> $outfile
